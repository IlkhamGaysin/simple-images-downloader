...
RSpec.describe ImageDownloader do
  it 'has a version number' do
    expect(ImageDownloader::VERSION).to eql('1.0.0')
  end
  ...
  describe '.from_file', :vcr do
    ...
    before do
      ...
      ImageDownloader::Configuration.destination = fixtures_path('image-fetcher/')
    end
    ...
    context 'when there is ImageDownloader::Errors::RedirectError raised for an yielded line' do
  ...
  describe '.from_url' do
    ...
    context 'when trere is no request' do
      ...
      let(:line)       { instance_double('ImageDownloader::Line') }
      let(:downloader) { instance_double('ImageDownloader::Downloader') }

      it do
        expect(ImageDownloader::Line).to receive(:new).with(url).and_return(line)
        expect(line).to receive(:uri).and_return(uri)
        expect(ImageDownloader::Downloader).to receive(:new).with(uri).and_return(downloader)
