...
RSpec.describe ImageDownloader::Errors do
  describe ImageDownloader::Errors::BaseError do
    it 'has custom message' do
      expect { raise described_class }.to raise_error(described_class)
    end
  end

  describe ImageDownloader::Errors::MissingFileArgumentError do
    it 'has custom message' do
      expect { raise described_class }.to raise_error(described_class, 'First arguments must be file')
    end
  end

  describe ImageDownloader::Errors::MissingFileError do
    let(:path) { '/test/test' }
  ...
  describe ImageDownloader::Errors::BadUrl do
  ...
  describe ImageDownloader::Errors::MissingImageInPath do
  ...
  describe ImageDownloader::Errors::RedirectError do
  ...
  describe ImageDownloader::Errors::ConnectionError do
    let(:uri) { URI.parse(Faker::Internet.url(path: '/test-image.png')) }
  ...
  describe ImageDownloader::Errors::DestinationIsNotWritable do
    let(:path) { '/test/test-image.png' }
