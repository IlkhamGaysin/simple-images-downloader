...
RSpec.describe ImageFetcher do
  it 'has a version number' do
    expect(ImageFetcher::VERSION).to eql('1.0.0')
  end
  ...
  describe '.from_file', :vcr do
    ...
    before do
      ...
      ImageFetcher::Configuration.destination = fixtures_path('image-fetcher/')
    end
    ...
    context 'when there is ImageFetcher::Errors::RedirectError raised for an yielded line' do
  ...
  describe '.from_url' do
    ...
    context 'when trere is no request' do
      ...
      let(:line)       { instance_double('ImageFetcher::Line') }
      let(:downloader) { instance_double('ImageFetcher::Downloader') }

      it do
        expect(ImageFetcher::Line).to receive(:new).with(url).and_return(line)
        expect(line).to receive(:uri).and_return(uri)
        expect(ImageFetcher::Downloader).to receive(:new).with(uri).and_return(downloader)
